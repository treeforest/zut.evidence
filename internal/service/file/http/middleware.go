...
func authHandler(jwtMgr *jwt.JWTManager) func(c *gin.Context) {
	return func(c *gin.Context) {
		...
		if path != `/login` && path != `/register` {
			...
			if err := c.BindHeader(&authHeader); err != nil {
				errors(c, RequestErr, "not found token")
				c.Abort()
				return
			}
			if authHeader.Token == "" {
			...
			if err != nil {
				errors(c, RequestErr, "invalid token")
				c.Abort()
				return
			}
			// 设置上下文元数据
